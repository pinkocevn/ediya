$(function(){
   ...
   $('#btn-menu').click(function(){
      ...
      // 메뉴버튼 안의 텍스트를 menu 또는 close로 들어가도록 처리하기
      // 조건문 사용 

      // if($(this).find('i').text()=='menu'){ // 1번 째
      //    $(this).find('i').text('close');
      // }else{
      //    $(this).find('i').text('menu');
      // }

      // if($('body').hasClass('open')){ // 2번 째
      //    $(this).find('i').text('close');
      // }else{
      //    $(this).find('i').text('menu');
      // }

      // if($(this).is(':contains(menu)')){ // 3번 째
      //    $(this).find('i').text('close');
      // }else{
      //    $(this).find('i').text('menu');
      // }
   ...
   // 화면의 사이즈가 변경될 때 gnb의 상태 초기화 하기
      $(window).resize(function(){
         ...
         if (windowWidth <= 1100) { // pc가 아님 ~1100px
            $('.gnb .sub').hide();
         } else { // pc임
            $('.gnb .sub').show();
         }
   ...
   $('.gnb i').click(function(e){
      e. preventDefault();
   if($(this).parents('li').hasClass('active')){ // i의 부모인 li에 active 클래스가 있다면
      $('.gnb > li').removeClass('active'); // active를 지우고 그 밑에 있는 서브메뉴를 닫는다.
      $('.gnb .sub').slideUp();
   }else{
